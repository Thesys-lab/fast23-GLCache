...
impl Builder {
    ...
    /// let cache = L2Cache::builder().hash_power(17).build();
    ///
    /// // create a cache with a larger hashtable with room for ~1.8M items
    /// let cache = L2Cache::builder().hash_power(21).build();
    /// ```
    ...
    /// let cache = L2Cache::builder().heap_size(64 * MB).build();
    ///
    /// // create a cache with a 256MB heap
    /// let cache = L2Cache::builder().heap_size(256 * MB).build();
    /// ```
    ...
    /// let cache = L2Cache::builder().segment_size(1 * MB).build();
    ///
    /// // create a cache using 4MB segments
    /// let cache = L2Cache::builder().segment_size(4 * MB).build();
    /// ```
    ...
    /// let cache = L2Cache::builder().eviction(Policy::Random).build();
    ///
    /// // create a cache using a merge based eviction policy
    /// let policy = Policy::Merge { max: 8, merge: 4, compact: 2};
    /// let cache = L2Cache::builder().eviction(policy).build();
    /// ```
    pub fn eviction(mut self, policy: Policy) -> Self {
        self.segments_builder = self.segments_builder.eviction_policy(policy);

        if cfg!(not(feature="oracle_reuse")) {
            info!("oracle_reuse disabled");
        } else {
            info!("oracle_reuse enabled");
            if cfg!(feature="offline_segment_utility") {
                info!("offline_segment_utility enabled");
            }
        }

        if cfg!(not(feature="oracle_item_selection")) {
            info!("oracle_item_selection disabled");
        } else {
            info!("oracle_item_selection enabled");
        }
